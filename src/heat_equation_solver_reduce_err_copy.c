...
#pragma acc loop independent collapse(2)
    ...
    for (curr_iter = 0u; curr_iter < max_iter && err > etol; curr_iter += N_ERR_COMPUTING_IN_DEVICE)
    {

      for (size_t n_err_iter = 0; n_err_iter + 1 < N_ERR_COMPUTING_IN_DEVICE; ++n_err_iter)
      {
...
#pragma acc loop independent collapse(2)
      ...
      err = (FLOAT_TYPE)0.0;
#pragma acc data present(next_grid [0:grid_sqr], curr_grid [0:grid_sqr]) copy(err)
#pragma acc kernels
      {
